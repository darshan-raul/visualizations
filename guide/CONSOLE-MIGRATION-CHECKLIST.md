...
| --- | --- | --- |
| Linux Foundations — `/linux` hub plus `/linux/identity`, `/linux/storage`, `/linux/processes`, `/linux/networking`, `/linux/packages`, `/linux/shell`, `/linux/security`, `/linux/troubleshooting` | Reopened after quality rejection and rebuilt as a complete eight-chapter visual tutorial rather than a trace-led view gallery. The hub teaches the model → mechanism → evidence → practice loop and three curriculum phases. All eight pillars expose outcomes, kernel objects, applied practice, three progressive index groups, 64 visual lessons in source order, failure evidence, and two feedback-rich knowledge checks. Accuracy corrections cover pipes, credentials/capabilities, SSH public-key authentication, systemd stop semantics, network lookup questions, and audit evidence. Stable routes and fragments remain. Canonical [Spec](topics/linux-foundation/linux-foundation-visual-spec.md) and complete-narrative [Prototype](topics/linux-foundation/linux-foundation-visual-prototype.html) updated. `npm run verify` passed on 2026-09-18: Astro 0/0/0, 31 routes built, graph 22/159, and 36 generated pages passed route/anchor checks. | [ ] Browser QA remains: desktop, 320 px, and 200% zoom; keyboard/touch, focus, reduced motion, no JavaScript, direct fragments, Back/Forward, overflow, contrast, and console errors. Browser tooling was unavailable in the workspace. |
