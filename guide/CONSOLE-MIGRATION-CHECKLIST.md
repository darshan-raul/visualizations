...
| --- | --- | --- |
| Linux Foundations — `/linux` hub plus `/linux/identity`, `/linux/storage`, `/linux/processes`, `/linux/networking`, `/linux/packages`, `/linux/shell`, `/linux/security`, `/linux/troubleshooting` | Overhauled with authentic educational system models: interactive Host Architecture Canvas on `/linux` (User Space ↔ Syscalls ↔ Kernel Subsystems ↔ Hardware), plus deep domain-specific architectural models across all 8 pillar subpages and 58 topics: VFS pipelines, dual capacity meters (blocks vs inodes), open-unlinked file leaks, traversal gates, 64-bit virtual address space maps, dynamic linker pipelines, package/service state boards, shell 9-phase expansion pipelines, file descriptor routing tables, DAC match ladders, network packet defense boundaries, USE method diagnostic matrices, and four resource cliffs. Passed `npm run verify` (`astro check`, `astro build`, `check:graph`, `check:links`). Updated [Spec](topics/linux-foundation/linux-foundation-visual-spec.md) and [Prototype](topics/linux-foundation/linux-foundation-visual-prototype.html). | [ ] Complete final browser QA at 320 px and 200% zoom, keyboard/touch, reduced motion, no JavaScript, all direct fragments, and back/forward; then record the result in the [Linux spec](topics/linux-foundation/linux-foundation-visual-spec.md). |
