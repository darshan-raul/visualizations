...
| Topic and route | Current state | Remaining checks |
| --- | --- | --- |
| Kubernetes API Server — `/kubernetes-api-server` | Canonical [MDX console](../src/content/topics/kubernetes-api-server.mdx): 11 grouped views with mechanism-specific request, watch, state, path, HA, and capability canvases; shared topology retained for request, watch, flow, actor, failure, and synthesis context; 23 selectable flows, 11 request-gate presets, 11 failure presets, actor filters, API internals, and a no-JavaScript reading path. [Spec](topics/kubernetes-api-server/kubernetes-api-server-visual-spec.md) · earlier [prototype](topics/kubernetes-api-server/kubernetes-api-server-visual-prototype.html). Direct production overhaul authorized on 2026-09-16; `npm run check`, `npm run build`, `npm run check:graph`, and `npm run check:links` pass. | [ ] Complete browser QA at 320 px and 200% zoom, desktop, keyboard/touch, reduced motion, no JavaScript, direct fragments, flow query state and Back/Forward. Firefox headless exited with code 139 for desktop and mobile captures on 2026-09-16, so visual QA is recorded as pending. |
