# How Kubernetes Turns API State Into Running Workloads — Visual specification

**Topic:** Kubernetes API Server. **Status:** Integrated redesign, version 1.1, 2026-09-16.  
**Audience:** Working developers learning Kubernetes from foundation through control-plane internals.  
**Page promise:** Follow a `web` Deployment from one accepted API request to three ready Pods, using a visual form suited to each request, watch, state transition, relationship, and failure.  
...
**Visual thesis:** Stored intent becomes runtime reality through independent loops. The API server anchors state coordination, while each view uses the smallest visual form that explains its mechanism.
...
| 7 | Boundaries / Control or data? | Does application traffic traverse kube-apiserver? | Side-by-side control-state and application-data paths | Control/data/both selector | API centrality describes state coordination, not ordinary application traffic. |
...
#### Composition
The opening view uses a deliberately reduced hub rather than squeezing the complete cluster topology into the first canvas. API clients sit left, kube-apiserver stays at the center with etcd directly below it, and controller manager, scheduler and kubelets form a quiet right-hand column. Each actor is a real button. Five subdued connectors establish the available relationships while the selected actor and one primary relationship become bright; only that relationship receives an inline action label. Cluster boundaries, a second worker, Pods, node-local runtime and extensions stay out of this foundation view and appear in later views when they matter. Initial state emphasizes clients → kube-apiserver, then actor selection replaces that focus. A status strip sits below the hub instead of obscuring it. The inspector keeps the view-level explanation concise until a reader selects an actor, then reveals reads/watches/writes/why. The bottom takeaway states that shared API objects connect these actors. Later views expand into the canonical topology while keeping kube-apiserver as the spatial anchor.
#### Interaction
...
#### Composition
The canonical topology remains at the top of the canvas with fixed actor positions and only the active request path emphasized. Its status sits in a dedicated strip below the drawing rather than covering nodes or connectors. A numbered gate rail follows beneath it. READ/WRITE comparison occupies a small top strip. A red failure stops at the exact gate, while later gates show `NOT REACHED`, not generic grey uncertainty. Inspector shows response, affected object and first operator clue. Motion follows only the selected request token; static step highlighting remains complete.
#### Interaction
...
#### Composition
The canonical topology stays above the teaching sequence so the active controller↔API relationship remains spatially grounded. The map is centered at a fixed internal coordinate system, receives the full main-column width, and places live status below the drawing. A concise introduction then establishes LIST as the starting snapshot and WATCH as the retained conversation. Four stages—LIST, WATCH, EVENT and RECONCILE—cross the canvas with only the selected stage raised. Client-side and server-side internals sit in two optional disclosures below; a separate recovery note explains `410 Gone`. The shared inspector follows the canvas at every desktop width so neither the map nor sequence is squeezed. Static stepping is clearer than perpetual animation.
#### Interaction
...
#### Composition
The centered canonical topology keeps the selected controller, scheduler or kubelet relationship visible. Below it, two flat bars show desired 3 and observed 0/1/3, followed by a compact Observe–Compare–Act rail and Pod object diff. Text on each selected object state names the actor that changed it. No giant nested object cards.
#### Interaction
...
#### Composition
The canonical topology stays aligned above a focused endpoint → three adjacent API replicas → etcd expansion. The selected dependency path in the map matches the state below it. One-replica fault greys API2 while API1/3 remain reachable; a separate Lease rail shows one active scheduler/controller-manager, not one active API server. No single replica is drawn as a universal leader.
#### Interaction
...
#### Composition
The canonical topology remains aligned above the cutaway and highlights the relevant external or backing relationship when one is selected. A flat capability rail groups Fundamental, Security, State, Performance, Extensibility and Operations; selected detail appears adjacent without populating 20 tiny labels simultaneously. Optional advanced depth shows watch cache and informer, APF queue/reject, audit stages, `/status`, `/scale`, `/log`, `/exec`, aggregation path, and external call boundaries.
#### Interaction
...
Shared-map alignment correction, 2026-09-16: full maps use a 1200 × 740 coordinate space with 190 × 76 actor cards (220 × 76 for clients and API). Integration actors and extensions occupy separate columns with at least 44 px between successive cards. Boundary headings have reserved space above the nodes. Connector endpoints meet card perimeters; the direct client/API relationship routes around the separate control-plane endpoint. A geometry check of all 18 actors found no card intersections, including an 8 px clearance margin, and no out-of-bounds cards. This verifies coordinate geometry; rendered text, browser zoom and interaction checks still require browser QA.
...
The existing self-contained `kubernetes-api-server-visual-prototype.html` remains an earlier approval artifact under `guide/topics/`, outside routing/catalogue/search. The user authorized the 2026-09-16 overhaul directly in production without a replacement prototype. The canonical implementation uses `src/content/topics/kubernetes-api-server.mdx`, `src/pages/kubernetes-api-server.astro`, neutral `TopicConsoleShell.astro`, composable `src/components/k8s/api-server/` topic modules, shared tokens and `src/styles/api-server-console.css`. Request gates and watches retain the shared topology above their distinct mechanism diagrams so every selected request or watch stage still lights its actors and directed path. Spec/status, control/data separation, HA, and capabilities use standalone visual forms; flow, component, and failure exploration also retain the topology. Browser visual QA remains pending and is recorded below rather than treated as passed.
...
Verification record, 2026-09-16: `npm run check` completed with zero diagnostics; the production build generated 31 routes and Pagefind indexed 36 pages; graph and internal-link checks passed. The overhaul preserves 11 unique view fragments, 23 selectable flows, 11 fault presets, grouped navigation, no-JavaScript reading order, and source links. Seven views retain either compact or full topology maps; six mechanism-specific canvases remain the primary explanation and some views combine both. The wide layout places the live inspector beside the canvas and collapses it below at narrower widths. View changes reset the persistent ribbon, flow query state applies only inside the flow view, recovery steps no longer render as failed paths, and Pod readiness uses `status.conditions`. Firefox headless captures at 1600 × 1000 and 390 × 844 exited with code 139 before producing screenshots. Browser visual, interaction, zoom and touch QA therefore remain pending.
